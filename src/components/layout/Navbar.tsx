...
const Logo = () => (
  <svg width="40" height="46" viewBox="0 0 55 63" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const Navbar: React.FC = () => {
  ...
  const linkClass = "text-[11px] md:text-xs font-black italic tracking-[0.2em] text-zinc-400 hover:text-white transition-colors duration-300 uppercase cursor-pointer";
  const mobileLinkClass = "text-5xl md:text-6xl font-black italic uppercase tracking-tighter text-zinc-400 hover:text-white transition-all duration-300 cursor-pointer text-left";
    ...
    <>
      <nav 
        className={`fixed top-0 left-0 w-full z-[130] transition-all duration-500 ${
          ...
          scrolled && !isMenuOpen ? 'bg-black/90 backdrop-blur-xl' : 'bg-transparent'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            >
              <div className={`h-[2px] bg-white transition-all duration-300 ${isMenuOpen ? 'w-6 rotate-45 translate-y-2' : 'w-6'}`} />
              <div className={`h-[2px] bg-white transition-all duration-300 ${isMenuOpen ? 'opacity-0' : 'w-4 group-hover:w-6'}`} />
              <div className={`h-[2px] bg-white transition-all duration-300 ${isMenuOpen ? 'w-6 -rotate-45 -translate-y-2' : 'w-6'}`} />
            </button>
          ...
          <div className="absolute left-1/2 -translate-x-1/2">
            <button onClick={(e) => scrollToSection(e, 'hero')} aria-label="Back to top">
              <Logo />
            </button>
          </div>
          ...
          <div className="flex items-center gap-8">
            <div className="hidden lg:flex items-center gap-8">
              <button onClick={(e) => scrollToSection(e, 'experience')} className={linkClass}>Experience</button>
              <button onClick={(e) => scrollToSection(e, 'contact')} className={linkClass}>Contact</button>
            </div>
            <button 
      ...
      >
        <div className="h-full w-full overflow-y-auto flex flex-col px-8 md:px-24 pt-32 pb-12">
          <div className="text-zinc-500 text-[10px] font-black italic tracking-[0.5em] mb-10 border-l border-zinc-900 pl-4">NAVIGATION</div>
          
          <div className="flex flex-col gap-4 mb-20">
            <button onClick={(e) => scrollToSection(e, 'hero')} className={mobileLinkClass}>Home</button>
            <button onClick={(e) => scrollToSection(e, 'about')} className={mobileLinkClass}>About</button>
            <button onClick={(e) => scrollToSection(e, 'projects')} className={mobileLinkClass}>Work</button>
          ...
          <div className="mt-auto grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-zinc-900 pt-12">
            ...
            <div className="hidden md:block">
              <div className="text-zinc-800 text-[10px] font-black italic tracking-widest mb-6 uppercase">Legal</div>
              <p className="text-zinc-700 text-[10px] font-bold">© 2024 JOTHIVASAN. ALL RIGHTS RESERVED.</p>
            </div>
          </div>
        </div>
      </div>
